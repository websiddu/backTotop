    ...
    buttonPos: {
      ...
      bottom: 10,
		...
		default, this.options);
			...
			if(this.config.backTotopLink.length > 0) this.config.backTotopLink = this.config.backTotopLink.clone();



			this.config.backTotopLink.html(this.config.text).addClass(this.config.cssClass).appendTo(this.config.isWindow ? "body" : this.$elem).hide();
			...
			if(this.config.isWindow === true) {
				...
				console.log(this.$elem.position().left + this.$elem.innerWidth() - this.config.buttonPos.right - this.config.backTotopLink.width())
				cssOpts = {
					position: "absolute",
					top: this.$elem.position().top + this.$elem.innerHeight() - this.config.buttonPos.bottom + "px",
					left: (
					this.config.buttonPos.left === "" ? this.$elem.position().left + this.$elem.innerWidth() - this.config.buttonPos.right - this.config.backTotopLink.width() + "px" : this.config.buttonPos.left + boxMargin + boxPadding) + "px"
				}
			}
		...
		focusOuthide: function() {
			...
			if(this.config.focusOuthide === true && this.config.isWindow != true) {
				this.$elem.bind("mouseleave", function(e) {
					...
					if(thiz.$elem.scrollTop() > thiz.config.scrollBottomOffset) thiz.showBackTotopButton(thiz.config.focusOuthideShowTime);
				});
			}
		},
